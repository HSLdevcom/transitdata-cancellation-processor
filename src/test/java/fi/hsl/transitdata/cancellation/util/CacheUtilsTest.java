...
public class CacheUtilsTest {
    ...
    private List<InternalMessages.TripCancellation> buildTripCancellations() {
        InternalMessages.TripCancellation tripCancellation1 = InternalMessages.TripCancellation.newBuilder()
                .setTripId("trip1").setSchemaVersion(1).setStatus(InternalMessages.TripCancellation.Status.CANCELED)
                .build();
        InternalMessages.TripCancellation tripCancellation2 = InternalMessages.TripCancellation.newBuilder()
                .setTripId("trip2").setSchemaVersion(1).setStatus(InternalMessages.TripCancellation.Status.CANCELED)
                .build();
    ...
    public void testSingleCancellationDataIsSavedToCache() {
        Cache<String, Map<String, CancellationData>> bulletinsCache = Caffeine.newBuilder()
                .expireAfterAccess(Duration.ofHours(4)).build(key -> new HashMap<>());
    ...
    public void testModifiedCancellationWithAddedCancellationIsCorrectlySaved() {
        InternalMessages.TripCancellation tripCancellation3 = InternalMessages.TripCancellation.newBuilder()
                .setTripId("trip3").setSchemaVersion(1).setStatus(InternalMessages.TripCancellation.Status.CANCELED)
                .build();
