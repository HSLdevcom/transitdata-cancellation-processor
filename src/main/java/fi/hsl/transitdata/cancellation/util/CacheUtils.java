...
public class CacheUtils {
     ...
     */
    public static List<CancellationData> handleBulletinCancellations(String bulletinId,
        ...
        if (tripCancellationDataInCache == null || tripCancellationDataInCache.isEmpty()) {
            ...
            for (String tripId : tripCancellationDataInCache.keySet()) {
                if (!newTripCancellationDataMap.containsKey(tripId)) {
                    ...
                    InternalMessages.TripCancellation tripCancellationToBeCancelled = tripCancellationDataInCache
                            .get(tripId).getPayload();
                    ...
                    CancellationData cancelledCancellationData = new CancellationData(cancelledTripCancellation,
                            cachedCancellationData.getTimestamp(), cachedCancellationData.getDvjId(),
            ...
            log.info(
                    "Bulletin modified. Previous version had {} cancellations. New version has {} cancellations: "
                            + "unchanged {}, new {}, cancellations-of-cancellations {}",
                    originalNumberOfCancellationsInCache, modifiedCancellationDataList.size(),
                    unchangedCancellationDataList.size(), newCancellationDataList.size(),
                    cancelledCancellationDataList.size());
        }

        // check
        if (getTripCancellationMapNoNull(bulletinId, bulletinsCache).keySet().size() != modifiedCancellationDataList
                .size()) {
            log.warn(
                    "Number of cancellations in bulletin does is not equal to number of cancellations in cache. BulletinId={}",
                    bulletinId);
        }
     ...
     * identifier)
     */
    public static Map<String, CancellationData> getTripCancellationMap(String bulletinId,
            Cache<String, Map<String, CancellationData>> bulletinsCache) {
     ...
     */
    public static Map<String, CancellationData> getTripCancellationMapNoNull(String bulletinId,
            Cache<String, Map<String, CancellationData>> bulletinsCache) {
